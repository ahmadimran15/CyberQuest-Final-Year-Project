...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        {
            // Avoid mouse looking if the game is paused
            if (Mathf.Abs(Time.timeScale) < float.Epsilon) return;

            // Get the rotation before it's changed
            float oldYRotation = transform.eulerAngles.y;

            // Get joystick input for right controller (Quest 3)
            float lookX = Input.GetAxis("RightJoystickHorizontal"); // Left/Right look
            float lookY = Input.GetAxis("RightJoystickVertical");   // Up/Down look

            // Apply camera rotation using right joystick input
            Vector2 lookInput = new Vector2(lookX, -lookY); // Invert Y if needed
            mouseLook.LookRotation(transform, cam.transform, lookInput);

            if (m_IsGrounded || advancedSettings.airControl)
            {
                // Rotate the rigidbody velocity to match the new direction
